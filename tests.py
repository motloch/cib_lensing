...
kappa = 0.01 * cos(theta) that can be worked out analytically
"""
...
# Check the two approaches match
try:
...
"""
Test the deflection code on a random sample of unlensed positions
"""

# Random unlensed positions
...
# Lensed positions obtained analytically using 'pen and paper'
...
"""
Test deflection angle components d (amplitude) and alpha (deviation) for a simple case of
kappa = 0.01 * cos(theta) rotated by 45 degrees that can be solved analytically.
"""
...
"""
Test the actual deflection code on a random sample of unlensed positions for the rotated
magnification from the previous test.
"""
...
"""
Check that the main code that combines lensing deflections and searching for the
appropriate healpix pixel works as expected. Also check at least some points move between
pixels.
"""
# Random unlensed positions
...
# Lensing map for which we can calculate the deflection maps analytically
NSIDE = 512
A = 0.01
th, ph = hp.pix2ang(NSIDE, range(12*NSIDE**2))
kappa_map = A*cos(th)
...
#These are lensed positions obtained by pen and paper
...
total_flux_MJy = s*pixel_size

try:
	assert_close(total_flux_MJy, 4., 0.2)
	print('All tests passed')
except:
	print('Something went wrong in test 6')
...
try:
	...
	print('Something went wrong in test 7')
...
try:
	assert(np.sum(pavel_map[:100]**2) > 0)
	assert(np.sum(pavel_map[:100]**2) != np.sum(wrong_map[:100]**2))
	print('All tests passed')
except:
	print('Something went wrong in test 8')
